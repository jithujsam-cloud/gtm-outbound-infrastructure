...
export default async function PublicClientDashboard({ params }: Props) {
  ...
  const campaignData = row ? {
    ...
    positive_reply_rate: (row.positive_reply_rate as number) ?? 0,
    ...
    last_synced: (row.last_synced as string) ?? new Date().toISOString(),
  } : null;

  const agencyName = client.agency_name ?? null;
  const agencyLogoUrl = client.agency_logo_url ?? null;
